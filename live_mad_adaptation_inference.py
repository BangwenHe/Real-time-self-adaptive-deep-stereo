...
if __name__ == "__main__":
    model = MADNet(weight_path="checkpoints/MADNet/kitti/weights.ckpt", image_shape=[640, 480])
    stereo_calibration_file = f"../AnyNet/calib_result/mate40pro.yml"
    
    # root_image_folder = "data/result"
    root_image_folder = "data/madnet_images/rgb_1657789702514"
    Q = load_stereo_coefficients(stereo_calibration_file)[-1]

    left_image_paths, right_image_paths = build_input_images(root_image_folder)
    output_folder = "results/test_inference_grassland2"
    os.makedirs(output_folder, exist_ok=True)

    for idx, (left_image_path, right_image_path) in enumerate(tqdm(zip(left_image_paths, right_image_paths))):
        imgL = load_image(left_image_path)[np.newaxis, ...]
        imgR = load_image(right_image_path)[np.newaxis, ...]

        disp = model.inference(imgL, imgR)
        ...
        depth_map = np.clip(depth_map, None, 25)
        ...
        cv2.imwrite(os.path.join(output_folder, os.path.basename(left_image_path)), 
            np.hstack([left_image, disp_vis, depth_map]))
        # cv2.imwrite(os.path.join(output_folder, os.path.basename(left_image_path)), depth_map)
