...
def main(args):
    ...
    if args.mode == 'MAD':
        ...
        print(f'left image shape: {inputs_modules["left"].shape}')
        print(f'right image shape: {inputs_modules["right"].shape}')
        print(f'disp image shape: {inputs_modules["target"].shape}')
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        try:	
            ...
            while True:
                ...
                perfs.append(run_end - run_start)
                new_loss = fetches[2]
            ...
            print(f'latency: {sum(perfs) / len(perfs)}')
            print(f'latency: {sum(perfs[-100:]) / len(perfs[-100:])}')
            with open(os.path.join(args.output, "latency.txt"), "w") as f:
                for i in perfs:
                    f.write(f'{i}\n')
